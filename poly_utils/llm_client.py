"""
Generalized LLM Client for Poly-Maker

Unified interface for multiple LLM providers with rate limiting, retry logic,
and cost tracking. Can be used by any bot in the poly-maker suite.

Supported providers:
- Google Gemini (recommended for speed + cost)
- DeepSeek (very competitive pricing)
- Anthropic Claude (high quality)
- OpenAI GPT (widely compatible)
...
Usage:
    ...
    response = client.query(prompt, json_mode=True)
...
logger = get_logger('poly_utils.llm_client')
...
class BaseLLMProvider(ABC):
    ...
    @abstractmethod
    def call(self, prompt: str, json_mode: bool = False, **kwargs) -> Dict[str, Any]:
        """
        Make LLM API call.
...
class OpenAIProvider(BaseLLMProvider):
    ...
    def call(self, prompt: str, json_mode: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Some models (like gpt-5-nano, o1) don't support custom temperature
            if 'nano' not in self.model.lower() and 'o1' not in self.model.lower():
                request_kwargs['temperature'] = kwargs.get('temperature', 0.3)
...
class LLMClient:
    ...
    Provides a consistent API across multiple LLM providers with built-in
    ...
    def __init__(self, config: Dict[str, Any], bot_name: Optional[str] = None):
        ...
        Args:
            ...
            bot_name: Optional bot name for logging (e.g., 'near_sure', 'neg_risk_arb')
        """
        self.config = config
        ...
        logger.info(f"Initialized LLMClient for '{self.bot_name}' using provider: {self.provider_name}")
        ...
        # Per-minute rate limiting (default 15 RPM for Gemini free tier)
    ...
    def query(self, prompt: str, json_mode: bool = False, retries: Optional[int] = None, **kwargs) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dict with:
                ...
                - content: The response content (str or dict if json_mode)
        ...
        for attempt in range(retries + 1):
            response = self.provider.call(prompt, json_mode=json_mode, **kwargs)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict with provider, total_calls, estimated_cost, hourly_calls, rpm_current, rpm_limit
        """
