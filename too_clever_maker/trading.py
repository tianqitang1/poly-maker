"""
Too Clever Maker Trading Logic - Current Enhanced Strategy
More defensive, complex market making with:
- Dynamic position sizing with 0.95/0.9 thresholds
- Full position stop-loss (vs partial)
- Early reverse position checks
- Market trend tracking for take-profit
- Over-sizing prevention at multiple layers
"""
...
if not os.path.exists('too_clever_maker/positions/'):
    os.makedirs('too_clever_maker/positions/')
    ...
    async with market_locks[market]:
        try:
            ...
            for detail in deets:
                ...
                fname = 'too_clever_maker/positions/' + str(market) + '.json'
                ...
                # ------- TAKE PROFIT / SELL ORDER MANAGEMENT -------
                    ...
                    market_state_file = f'too_clever_maker/positions/{market}_{detail["name"]}_market.json'
