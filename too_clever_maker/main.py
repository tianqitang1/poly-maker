"""
Too Clever Maker Bot - Current Enhanced Strategy
Uses the current market making strategy with all enhancements
"""
...
def cleanup_inactive_markets():
    ...
    try:
        ...
        for token, condition_id in tokens_to_cleanup:
            ...
            print(f"\nToken {token_str} (market {condition_id[:16] if condition_id else 'unknown'}...):")
...
async def main():
    ...
    """
    # Initialize logger
    logger = get_logger('too_clever_maker')
    global_state.logger = logger

    # Initialize client with TOO_CLEVER_MAKER credentials
    pk = os.getenv('TOO_CLEVER_MAKER_PK')
    browser_address = os.getenv('TOO_CLEVER_MAKER_BROWSER_ADDRESS')

    if not pk or not browser_address:
        logger.error("Missing TOO_CLEVER_MAKER_PK or TOO_CLEVER_MAKER_BROWSER_ADDRESS environment variables!")
        print("\nERROR: Missing TOO_CLEVER_MAKER_PK or TOO_CLEVER_MAKER_BROWSER_ADDRESS environment variables!")
        print("Please add these to your .env file:")
        print("TOO_CLEVER_MAKER_PK=your_private_key_here")
        print("TOO_CLEVER_MAKER_BROWSER_ADDRESS=your_wallet_address_here")
        return
    ...
    logger.info("")
    logger.info(f'[TOO CLEVER MAKER] There are {len(global_state.df)} markets, {len(global_state.positions)} positions and {len(global_state.orders)} orders.')
